...
| Hard flick (45px per frame for half a second) | peaks ~73px about 7 frames after the scroll stops, flat again ~3.8s later |
...
The original lives in Louis's private labs repo (`gooey-sections.html`, tab 4), served at http://localhost:8094 on Louis's Mac. If you can open it, compare side by side.
...
3. Load the script after the markup, then start it:
   ...
   <script>GooeyEdge.init({ variant: 'honey' });</script>
   ```
...
To mix feels on one page, give a section its own value, for example `data-goo-edge="goo"`.

### Webflow

- On each section, open Element settings and add a custom attribute named `data-goo-edge` with the value `honey`. On a Lumos site, that's the `section_<name>` element.
- Webflow can't host .js files as assets, so paste the engine inline. In Site settings or Page settings, go to Custom code, Footer (Before `</body>` tag). Add `<script>`, the full contents of `assets/gooey-section.js`, then `</script>`, followed by `<script>GooeyEdge.init({ variant: 'honey' });</script>`. The engine is about 17,000 characters and the field holds 50,000, so it fits alongside other code.
...
export default function GooeyEdges({ variant = 'honey' }) {
  useEffect(() => {
...
Render `<GooeyEdges variant="honey" />` once, after the sections, in the page that renders them. Keeping the effect in this leaf means an app-router page can stay a Server Component. Mark the sections with `data-goo-edge="honey"` (or `""`); a bare `data-goo-edge` renders as `"true"`, which also falls back to the component's variant. React Strict Mode's double effect is fine, because `destroy()` removes everything. On client-side route changes the component unmounts and remounts with the new page, so each page gets fresh edges.
...
- Scroll hard, then stop: the blob keeps rising briefly after you stop, then sinks very slowly, still full and round.
...
Leave the numbers alone unless the user asks for a different feel, because they match the lab. If asked, these are the knobs for Honey. The full table is in `references/engine.md`.

- Heaviness: `drive.down` 0.018 (release) and `drive.up` 0.08 (rise).
- Lag: `drive.demand` 0.06.
- Fullness: the 0.65 exponent (lower gives a flatter, broader top).
- Strength: `vel` 2.6.

Tune by adding a variant rather than editing one in place, so the original stays available to compare against:

```js
GooeyEdge.variants['honey-soft'] = { ...GooeyEdge.variants.honey, vel: 1.95 }; // was 2.6
GooeyEdge.init({ variant: 'honey-soft' });
```

Copy `drive` too (`drive: { ...GooeyEdge.variants.honey.drive, … }`) before changing its numbers, so the original isn't mutated.
