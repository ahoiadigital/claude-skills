...
| Hard flick (45px per frame for half a second) | rises ~89px on one side and dips as deep on the other, flat again ~1.7s later |
...
The original lives in Louis's private labs repo (`gooey-sections.html`, tab 8), served at http://localhost:8094 on Louis's Mac. If you can open it, compare side by side.
...
3. Load the script after the markup, then start it:
   ...
   <script>GooeyEdge.init({ variant: 'slosh' });</script>
   ```
...
To mix feels on one page, give a section its own value, for example `data-goo-edge="peel"`.

### Webflow

- On each section, open Element settings and add a custom attribute named `data-goo-edge` with the value `slosh`. On a Lumos site, that's the `section_<name>` element.
- Webflow can't host .js files as assets, so paste the engine inline. In Site settings or Page settings, go to Custom code, Footer (Before `</body>` tag). Add `<script>`, the full contents of `assets/gooey-section.js`, then `</script>`, followed by `<script>GooeyEdge.init({ variant: 'slosh' });</script>`. The engine is about 17,000 characters and the field holds 50,000, so it fits alongside other code.
...
export default function GooeyEdges({ variant = 'slosh' }) {
  useEffect(() => {
...
Render `<GooeyEdges variant="slosh" />` once, after the sections, in the page that renders them. Keeping the effect in this leaf means an app-router page can stay a Server Component. Mark the sections with `data-goo-edge="slosh"` (or `""`); a bare `data-goo-edge` renders as `"true"`, which also falls back to the component's variant. React Strict Mode's double effect is fine, because `destroy()` removes everything. On client-side route changes the component unmounts and remounts with the new page, so each page gets fresh edges.
...
- Scroll hard, then stop: scrolling down, the edge climbs on the cursor's left and sinks on its right; scrolling up reverses it.
...
Leave the numbers alone unless the user asks for a different feel, because they match the lab. If asked, these are the knobs for Slosh. The full table is in `references/engine.md`.

- Width of the lean: `sig` 300 + 0.7.
- Strength: `vel` 2.4.
- Direction: negate the profile to lean the other way.
- The 1.6487 factor normalises the peak to 1. Leave it.

Tune by adding a variant rather than editing one in place, so the original stays available to compare against:

```js
GooeyEdge.variants['slosh-soft'] = { ...GooeyEdge.variants.slosh, vel: 1.8 }; // was 2.4
GooeyEdge.init({ variant: 'slosh-soft' });
```

Copy `drive` too (`drive: { ...GooeyEdge.variants.slosh.drive, … }`) before changing its numbers, so the original isn't mutated.
